...
하지만 일반인들에게 병원에 직접 가서 검사 받기에는 번거롭고 몸에 전극 여러개 붙이고 낯선 환경에서 잠을 자야되서 불편합니다.

수면다원검사는 여러가지 데이터를 측정하는데, 그 중 측정하기 쉬운 ECG데이터와 호흡데이터가 있습니다.

비접촉 Radar 센서를 이용하여 얻을 수 있는 ECG데이터와 호흡데이터를 이용하여 수면 단계를 예측할 수 있는 모델을 만들 수 있게 된다면 집에서도 좋은 수면을 하고 있는지 알 수 있게 될 것입니다.
# 프로젝트 진행 과정
1. 도메인 공부
2. 데이터 공부
